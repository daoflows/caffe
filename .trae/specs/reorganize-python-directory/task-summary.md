...
> **任务名称**：`python/` 目录系统性整理与模块分包 + pycaffe 子目录优化  
> **执行日期**：2026-07-23  
> **报告版本**：标准版 v2  
...
| 维度 | 内容 |
|------|------|
| **任务目标** | 将 `python/` 目录下混乱的 12 个顶层文件按功能模块重组为 4 个子包 |
...
### 初始问题
`python/` 目录顶层混合了 4 种类型的文件（库模块、生成代码、脚本、测试），外加命名不清晰（`utils.py` 实际是 TVM Relax 算子），缺少包层级组织。
...
│   ├── python/pycaffe/                     │   └── layers.py          ← 原 utils.py
...
### 约束条件
- `python/pycaffe/` 子目录（独立轮子构建项目）不受影响
- `python/protos/` 保留作为 gen_proto.py 输出目标
...
│   └── caffe_pb2.py
├── pycaffe/                 # 独立子项目（已优化）
│   ├── python/pycaffe/      # pycaffe 源码包（9 文件，-25%）
...
| 1.2 | 全面分析 `python/` 目录结构 | 识别 4 类文件、12 个待移动文件 |
...
| 引用 | 旧路径 | 新路径 |
|------|--------|--------|
| gen_proto.py 执行命令 | `python python/gen_proto.py` | `python python/scripts/gen_proto.py` |
| caffe_pb2.py 输出位置 | `python/caffe_pb2.py` | `python/caffeproto/caffe_pb2.py` |
| utils.py 引用 | `python/utils.py` | `python/operators/layers.py` |
...
| 步骤 | 操作 | 结果 |
|------|------|------|
| 7.1 | 删除根目录 `protos/caffe.proto` 残留 | 旧文件已迁移到 `python/protos/` |
| 7.2 | 更新 `README.md` protoc 命令路径 | `protos/caffe.proto` → `python/protos/caffe.proto` |
...
| 8.4 | 移动 `test_inference.py`、`verify.py` → `python/tests/` | 测试统一管理 |
| 8.5 | 移动 `_check_python.sh`、`_diag.sh` → `python/scripts/` | 脚本统一管理 |
...
| 类型 | 文件 | 操作 |
|------|------|------|
| 新增 | `python/caffeproto/__init__.py` | 创建 |
| 新增 | `python/caffeproto/caffe_utils.py` | 从 `python/caffe_utils.py` rename |
| 新增 | `python/caffeproto/caffe_fuse.py` | 从 `python/caffe_fuse.py` rename |
| 新增 | `python/operators/__init__.py` | 创建 |
| 新增 | `python/operators/layers.py` | 从 `python/utils.py` rename |
| 新增 | `python/scripts/gen_proto.py` | 从 `gen_proto.py` rename |
| 新增 | `python/scripts/run_test.sh` | 从 `run_test.sh` rename |
| 新增 | `python/scripts/test_new_features.sh` | 从 `test_new_features.sh` rename |
| 新增 | `python/tests/__init__.py` | 创建 |
| 新增 | `python/tests/test_l2norm.py` | 从 `python/test_l2norm.py` rename |
...
| 类型 | 文件 | 操作 |
|------|------|------|
| 删除 | `protos/caffe.proto` | 迁移到 `python/protos/caffe.proto` |
| 新增 | `python/protos/caffe.proto` | 从 `protos/caffe.proto` rename |
...
| 迁移 | `test_inference.py` | `pycaffe/` → `python/tests/` |
| 迁移 | `verify.py` | `pycaffe/` → `python/tests/` |
| 迁移 | `_check_python.sh` | `pycaffe/` → `python/scripts/` |
| 迁移 | `_diag.sh` | `pycaffe/` → `python/scripts/` |
