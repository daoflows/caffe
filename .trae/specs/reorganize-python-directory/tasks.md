...
- [x] Task 1: 创建新目录结构
  - [x] 创建 `python/caffeproto/`、`python/operators/`、`python/scripts/`、`python/tests/` 目录
  - [x] 为 `caffeproto/`、`operators/`、`tests/` 创建 `__init__.py`

- [x] Task 2: 移动文件到新位置
  - [x] `python/caffe_pb2.py` → 删除（由 gen_proto.py 生成到 `caffeproto/`）
  - [x] `python/caffe_utils.py` → `python/caffeproto/caffe_utils.py`
  - [x] `python/caffe_fuse.py` → `python/caffeproto/caffe_fuse.py`
  - [x] `python/utils.py` → `python/operators/layers.py`
  - [x] `python/gen_proto.py` → `python/scripts/gen_proto.py`
  - [x] `python/run_test.sh` → `python/scripts/run_test.sh`
  - [x] `python/test_new_features.sh` → `python/scripts/test_new_features.sh`
  - [x] `python/test_l2norm.py` → `python/tests/test_l2norm.py`
...
- [x] Task 5: 更新 README.md 中的路径引用
  - [x] `python python/gen_proto.py` → `python python/scripts/gen_proto.py`（3 处）
  - [x] `test_l2norm.py` 引用 → `tests/test_l2norm.py`（1 处）
  - [x] `python/caffe_pb2.py` → `python/caffeproto/caffe_pb2.py`（1 处）
  - [x] `python/utils.py` → `python/operators/layers.py`（1 处）
