...
| 原依赖 | tvm-ffi替代 | 兼容层位置 |
|--------|-------------|-----------|
| `glog/logging.h` (LOG/CHECK) | `TVM_FFI_THROW`/`TVM_FFI_CHECK` | [include/caffe/compat/logging.hpp](file:///d:/spaces/SpecWeave/external/chaos/caffe/python/include/caffe/compat/logging.hpp) |
| `boost::shared_ptr` | `std::shared_ptr` | 标准库替代 |
| `boost::mutex`/`condition_variable` | `std::mutex`/`std::condition_variable` | [include/caffe/compat/thread.hpp](file:///d:/spaces/SpecWeave/external/chaos/caffe/python/include/caffe/compat/thread.hpp) |
| `boost::python` | `TVM_FFI_DLL_EXPORT_TYPED_FUNC` + DLPack | [src/caffe/_caffe.cpp](file:///d:/spaces/SpecWeave/external/chaos/caffe/python/src/caffe/_caffe.cpp) |
| `boost::filesystem` | 精简兼容层 | [include/caffe/compat/filesystem.hpp](file:///d:/spaces/SpecWeave/external/chaos/caffe/python/include/caffe/compat/filesystem.hpp) |
...
| 优先级 | 行动项 | 验收标准 | 状态 | 完成日期 |
|--------|--------|---------|------|---------|
| 中 | 为C++依赖瘦身类任务创建spec模板 | 模板包含环境预检、API映射表、依赖闭包分析等检查项 | ✅ 已完成 | 2026-07-23 |
| 低 | 验证compat/兼容层模式在其他C++项目中的可复用性 | 至少在1个其他依赖替换项目中成功应用 | ⏳ 待验证 | - |
| 低 | pycaffe/CMakeLists.txt更新为tvm-ffi版本（当前legacy的boost配置可保留用于caffex） | 可选，如pycaffe独立构建需要 | ✅ 已完成 | 2026-07-24 |

**行动项完成说明**：

- **C++依赖瘦身spec模板**：已在 [spec/templates/cpp-dependency-slimming/](file:///d:/spaces/SpecWeave/spec/templates/cpp-dependency-slimming/) 创建完整模板（README.md + spec.md + tasks.md + checklist.md），包含Step 0/0.5/1强制前置检查、增量构建规则、反模式检查等实战萃取内容
- **pycaffe CMakeLists.txt**：已更新 [pycaffe/CMakeLists.txt](file:///d:/spaces/SpecWeave/external/chaos/caffe/python/pycaffe/CMakeLists.txt) 为tvm-ffi版本；旧boost::python版本保留备份（文件内头部注释说明）。新CMake支持scikit-build-core wheel构建，自动复制libtvm_ffi.so到包目录
