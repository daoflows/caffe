    ...
    if input_dict:
        for name, arr in input_dict.items():
            try:
                ...
                for input_blob in net.input_blobs_array():
...
def copy_from(self, trained_filename: Union[str, Path]) -> None:
    ...
    if self._is_native:
        self.CopyTrainedLayersFrom(str(trained_filename))
    else:
...
def blobs_dict(self) -> Dict[str, Blob]:
    ...
    if self._is_native:
        ...
        for i, blob in enumerate(blobs):
            if i < len(names):
                result[str(names[i])] = blob
            else:
                result[f"blob_{i}"] = blob
    else:
        result = dict(self._py_blobs)
    return result
...
def layers_dict(self) -> Dict[str, Layer]:
    ...
    if self._is_native:
        ...
        for i, layer in enumerate(layers):
            if i < len(names):
                result[str(names[i])] = layer
            else:
                result[f"layer_{i}"] = layer
    else:
        result = dict(self._py_layers)
    return result
...
def _forward_pure_python(self, input_dict: Dict[str, np.ndarray]) -> Dict[str, np.ndarray]:
    ...
    result = {}
    for blob in self._py_output_blobs:
