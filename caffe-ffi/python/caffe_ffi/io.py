...
def read_net(
    ...
    if ffi.is_available() and caffemodel_path is None:
        ...
        if new_net_from_file is not None:
            return Net(handle=new_net_from_file(str(prototxt_path)))
    ...
    if ffi.is_available():
        ...
        if new_net_from_str is not None:
            ...
            return Net(handle=new_net_from_str(proto_text))
...
def _build_net_from_param(net: Net, param: caffe_pb2.NetParameter) -> None:
    ...
    net._name = param.name if param.name else ""
    net._blobs = {}
    net._layers = {}
    net._blob_list = []
    net._layer_list = []
    ...
    for input_name in param.input:
        ...
        if param.input_shape:
            for i, shape in enumerate(param.input_shape):
                if i == len(net._input_blobs):
                    dims = list(shape.dim)
                    blob.Reshape(dims)
                    break
        elif param.input_dim:
            blob.Reshape(list(param.input_dim))
        net._blobs[input_name] = blob
        net._blob_list.append(blob)
        net._input_blobs.append(blob)
    
    for layer_param in param.layer:
        layer = Layer()
        layer._name = layer_param.name
        layer._type_str = layer_param.type
        layer._blobs = []
        ...
        for blob_proto in layer_param.blobs:
            ...
            layer._blobs.append(blob)
        
        net._layers[layer_param.name] = layer
        net._layer_list.append(layer)
        
        for top_name in layer_param.top:
            if top_name not in net._blobs:
                blob = Blob()
                blob.name = top_name
                net._blobs[top_name] = blob
                net._blob_list.append(blob)
                if layer_param.type in ["Softmax", "SoftmaxWithLoss"]:
                    net._output_blobs.append(blob)
    
    if not net._output_blobs:
        for layer_param in reversed(param.layer):
            for top_name in layer_param.top:
                if top_name in net._blobs and top_name not in [b.name for b in net._input_blobs]:
                    net._output_blobs.append(net._blobs[top_name])
                    break
            if net._output_blobs:
                break
