...
import numpy as np
import pytest
from utils import _download_model, _preprocess_imagenet, _test_network


def _test_resnet50(data, model_dir):
    data_process = _preprocess_imagenet(data)
    ...
    proto_file = _download_model(proto_file_url, "resnet50.prototxt", model_dir)
    blob_file = _download_model(blob_file_url, "resnet50.caffemodel", model_dir)
...
@pytest.mark.slow
def test_forward_Resnet50(caffe_model_dir):
    """Resnet50"""
    data = np.random.randint(0, 256, size=(1, 3, 224, 224)).astype(np.float32)
    _test_resnet50(data, caffe_model_dir)
