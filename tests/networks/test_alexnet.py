...
def _test_alexnet(data, model_dir):
    ...
    proto_file = _download_model(proto_file_url, "alexnet.prototxt", model_dir)
    blob_file = _download_model(blob_file_url, "alexnet.caffemodel", model_dir)
...
@pytest.mark.slow
def test_forward_Alexnet(caffe_model_dir):
    """Alexnet"""
    data = np.random.randint(0, 256, size=(1, 3, 227, 227)).astype(np.float32)
    _test_alexnet(data, caffe_model_dir)
