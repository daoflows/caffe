...
#include "caffe/compat/filesystem.hpp"
...
void UpgradeSnapshotPrefixProperty(const string& param_file,
  ...
  if (!param->has_snapshot_prefix()) {
    param->set_snapshot_prefix(fs::path(param_file).replace_extension().string());
    LOG(INFO) << "snapshot_prefix was not specified and is set to "
                + param->snapshot_prefix();
  } else if (fs::is_directory(param->snapshot_prefix())) {
    param->set_snapshot_prefix((fs::path(param->snapshot_prefix()) /
                               fs::path(param_file).stem()).string());
