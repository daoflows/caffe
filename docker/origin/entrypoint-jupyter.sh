...
print_banner() {
    echo ""
    echo "============================================================"
    echo "  Caffe Jupyter SSH Container starting..."
    echo "  Time: $(date)"
    echo "  Host: $(hostname)"
    echo "============================================================"
    echo ""
...
diagnose_system() {
    ...
    log_info "User: $(id)"
    log_info "Non-root user: ${NON_ROOT_USER:-caffe-origin}"

    if [ -f /etc/caffe-jupyter-build-info ]; then
        log_info "Build info:"
        while IFS= read -r line; do log_info "  $line"; done < /etc/caffe-jupyter-build-info
    fi
    log_info "========================================"
    echo ""
}
...
setup_passwords() {
    ...
    if [ -n "${USER_PASSWORD:-}" ]; then
        echo "${NON_ROOT_USER:-caffe-origin}:${USER_PASSWORD}" | chpasswd
        log_info "${NON_ROOT_USER:-caffe-origin} password set from USER_PASSWORD env var"
    else
        USER_PASSWORD=$(pwgen -s 16 1)
        echo "${NON_ROOT_USER:-caffe-origin}:${USER_PASSWORD}" | chpasswd
        log_warn "USER_PASSWORD not set, generated random password for ${NON_ROOT_USER:-caffe-origin}"
        generated_password=1
    fi

    if [ "${GRANT_SUDO:-no}" = "yes" ]; then
        echo "${NON_ROOT_USER:-caffe-origin} ALL=(ALL) NOPASSWD:ALL" > "/etc/sudoers.d/${NON_ROOT_USER:-caffe-origin}"
        chmod 0440 "/etc/sudoers.d/${NON_ROOT_USER:-caffe-origin}"
        log_info "Sudo NOPASSWD enabled for ${NON_ROOT_USER:-caffe-origin}"
    fi
    ...
    if [ "$generated_password" = "1" ]; then
        ...
        echo "    * [IMPORTANT] ${NON_ROOT_USER:-caffe-origin} password: ${USER_PASSWORD}"
        echo "    * SSH login: ssh ${NON_ROOT_USER:-caffe-origin}@<host> -p <port>"
        echo "    ************************************************"
        echo ""
    fi
}
...
setup_ssh_keys() {
    ...
    local user="${NON_ROOT_USER:-caffe-origin}"
...
print_access_info() {
    ...
    echo ""
    echo "  SSH access:"
    echo "    ssh ${NON_ROOT_USER:-caffe-origin}@<host> -p <mapped-port>"
...
if [ $# -gt 0 ]; then
    ...
    echo ""
    if [ "$(id -u)" = "0" ]; then
        target_user="${NON_ROOT_USER:-caffe-origin}"
