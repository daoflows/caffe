...
print_banner() {
    echo ""
    echo "============================================================"
    echo "  PyCaffe Jupyter SSH Container starting..."
    echo "  Time: $(date)"
    echo "  Host: $(hostname)"
    echo "============================================================"
    echo ""
...
diagnose_system() {
    ...
    log_info "User: $(id)"
    log_info "Non-root user: ${NON_ROOT_USER:-builder}"
...
setup_passwords() {
    ...
    if [ -n "${USER_PASSWORD:-}" ]; then
        echo "${NON_ROOT_USER:-builder}:${USER_PASSWORD}" | chpasswd
        log_info "${NON_ROOT_USER:-builder} password set from USER_PASSWORD env var"
    else
        USER_PASSWORD=$(pwgen -s 16 1)
        echo "${NON_ROOT_USER:-builder}:${USER_PASSWORD}" | chpasswd
        log_warn "USER_PASSWORD not set, generated random password for ${NON_ROOT_USER:-builder}"
        generated_password=1
    fi

    if [ "${GRANT_SUDO:-no}" = "yes" ]; then
        echo "${NON_ROOT_USER:-builder} ALL=(ALL) NOPASSWD:ALL" > "/etc/sudoers.d/${NON_ROOT_USER:-builder}"
        chmod 0440 "/etc/sudoers.d/${NON_ROOT_USER:-builder}"
        log_info "Sudo NOPASSWD enabled for ${NON_ROOT_USER:-builder}"
    fi
    ...
    if [ "$generated_password" = "1" ]; then
        ...
        echo "    * [IMPORTANT] ${NON_ROOT_USER:-builder} password: ${USER_PASSWORD}"
        echo "    * SSH login: ssh ${NON_ROOT_USER:-builder}@<host> -p <port>"
        echo "    ************************************************"
        echo ""
    fi
}
...
print_access_info() {
    ...
    echo ""
    echo "  SSH access:"
    echo "    ssh ${NON_ROOT_USER:-builder}@<host> -p <mapped-port>"
