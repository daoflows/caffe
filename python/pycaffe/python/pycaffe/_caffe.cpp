...
BOOST_PYTHON_MODULE(_caffe) {
  // Initialize NumPy C API FIRST before any class registration that uses NumPy types.
  // import_array() must be called before using PyArray_Type or any NumPy C API functions,
  // otherwise get_pytype() in NdarrayConverterGenerator will segfault (NumPy API not ready).
