...
void caffe_rng_uniform(const int n, const Dtype a, const Dtype b, Dtype* r) {
  ...
  caffe::rng_t* generator = caffe_rng();
  for (int i = 0; i < n; ++i) {
...
void caffe_rng_bernoulli(const int n, const Dtype p, int* r) {
  ...
  CHECK_GE(p, 0);
  CHECK_LE(p, 1);
  std::bernoulli_distribution random_distribution(static_cast<double>(p));
  caffe::rng_t* generator = caffe_rng();
  for (int i = 0; i < n; ++i) {
    r[i] = random_distribution(*generator);
  }
...
void caffe_rng_bernoulli(const int n, const Dtype p, unsigned int* r) {
  ...
  caffe::rng_t* generator = caffe_rng();
  for (int i = 0; i < n; ++i) {
    r[i] = static_cast<unsigned int>(random_distribution(*generator));
  }
}
